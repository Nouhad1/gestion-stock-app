...
router.get('/cards', async (req, res) => {
  ...
  try {
    const [[entrees]] = await db.promise().query(`
      SELECT SUM(quantite_achat) AS totalEntrees,
             SUM(prix_achat * quantite_achat) AS valeurAchats
      FROM achats
      WHERE YEAR(date_achat) = ?
    `, [year]);

    const [[sorties]] = await db.promise().query(`
      SELECT SUM(quantite_commande) AS totalSorties,
...
router.get('/chart', async (req, res) => {
  ...
  try {
    ...
    // transformer le numéro de mois en label
