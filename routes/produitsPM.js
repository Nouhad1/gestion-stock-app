...
router.post('/', (req, res) => {
  ...
  db.query(sql, [reference, designation, qs, 0], (err, result) => {
    if (err) {
      if (err.code === 'ER_DUP_ENTRY') return res.status(400).json({ error: 'Référence déjà existante' });
...
router.put('/:reference', (req, res) => {
  ...
  db.query(sql, [designation, prix_unitaire, quantite_stock, quantite_stock_2, reference], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erreur lors de la mise à jour du produit' });
